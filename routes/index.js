...
function sentenceA(seed) {
  return aToAnIfNeeded(
    [randomArticle(seed++), randomNoun(seed++), randomVerb(seed++)].join(" ")
  );
}
...
function sentenceE(seed) {
  ...
  return (
    sentences[Math.floor(generator.random() * sentences.length)](seed++) +
    " because " +
    sentences[Math.floor(generator.random() * sentences.length)](10 + seed++)
  );
}
...
function randomSentence(seed) {
  ...
  sentence = sentences[Math.floor(generator.random() * sentences.length)](
    seed++
  );
...
function randomLink(seed, hostname) {
  ...
  var link = [];
  link["href"] =
    "/" + randomSentence(seed).replace(/ /g, "/").replace(/,/g, "");
...
function generateSeed(path) {
  ...
  var sum = md5(path);
  var seed =
    parseInt(sum.slice(0, 7), 16) +
    parseInt(sum.slice(8, 15), 16) +
    parseInt(sum.slice(16, 23), 16) +
    parseInt(sum.slice(24, 31), 16);
  return seed;
}
